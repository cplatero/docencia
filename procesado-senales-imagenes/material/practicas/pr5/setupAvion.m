...
% image = @(str) double(flipud(255*im2bw(imread(str)))'); % reads and converts
% image = @(str) double(flipud(im2bw(imread(str)))'); % reads and converts

% load the original data, set domain, initial discretization, and grid
% dataT = image('Avion/avion2.jpg');
% dataR = image('Avion/avion1.jpg');
dataR =im2bw(imread('Avion/avion1.jpg'));dataR([1,end],:)=false;
dataR = double(flipud(255*dataR)');
% dataR = double(flipud(convertSmoothModel(dataR))');

dataT =im2bw(imread('Avion/avion2.jpg'));dataT([1,end],:)=false;
dataT = double(flipud(255*dataT)');
% dataT = double(flipud(convertSmoothModel(dataT))');

omega = [0,1,0,1]; % [left,right,bottom,top]
m     = [64,64];
