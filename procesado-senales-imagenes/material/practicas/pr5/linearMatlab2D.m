% function [Tc,dT] = linearMatlab2D(T,omega,x,varargin);
% (c) Jan Modersitzki 2009/03/24, see FAIR.2 and FAIRcopyright.m.
% MATLAB's linear interpolator for the data T given on a cell-centered grid on
%  (omega(1),omega(2)) x (omega(3),omega(4)), to be evaluated at x
...
m  = size(T); h = (omega(2:2:end)-omega(1:2:end))./m; d = length(omega)/2; 
xi = @(i) (omega(2*i-1)+h(i)/2:h(i):omega(2*i)-h(i)/2)';
n = length(x)/d;                          
[X1,X2] = meshgrid(xi(1),xi(2));
Tfctn = @(x1,x2) interp2(X1,X2,permute(T,[2,1]),x1,x2,'linear');
